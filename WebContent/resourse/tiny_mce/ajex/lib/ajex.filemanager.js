...
var Action = {
	createFolder: function() {
		...
		dialogSet($lang.enterNameCreateFolder, '<b>' + $lang.location + '</b> [' + $cfg.url + ']<br /><input type="text" id="newName" value="" class="t" /><br />' + $lang.allowRegSymbol);
		return;
	},
	...
	renameFile: function() {
		...
		dialogSet($lang.enterNewNameFile, '<b>' + $cfg.file + '</b> [' + $cfg.url + ']<br /><input type="text" id="newName" value="" class="t" /><br />' + $lang.allowRegSymbol);
	},
	...
	lookFile: function() {
		if ('' == $cfg.file) return false;
		window.open($cfg.url + $cfg.file, 'preView', '');
	},
	setThumb: function() {
		if ('' == $cfg.file) return false;
	...
	setFile: function() {
		if ('' == $cfg.file) return false;
		_setReturnData($cfg.url + $cfg.file);
	}
...
{
	$('>div.l', statusDiv).html('<div>' + $cfg.url + '</div><div><b>' + reply.files.length + '</b> ' + $lang.fileOf + '</div>');

	var files = reply.files;
	var list = '', thumb = '', w_h = '', attr = '';

	for (var i in files) {
		attr = 'file="' + ($cfg.url + files[i].name) + '" thumb="' + (files[i].width? ($cfg.url + $cfg.thumb + '/' + $cfg.dir + '/' + files[i].name) : '') + '"';
	...
	$('#fileThumb > div.thumb').each(function() {
		...
		div.click(function() {
			...
			$('>div.l', statusDiv).html('<div class="cutName"><a href="' + $cfg.url + $cfg.file + '" target="_urlFile">' + $cfg.url + $cfg.file + '</a></div><div>'+$lang.fileSize+': '+$('.size', div).text()+'</div><div>'+$lang.fileDate+': '+$('.date', div).text()+'</div>');
			...
			_setReturnData($cfg.url + $('.name', div).text());
		});

		$('#fileList .name a').dblclick(function () {
			_setReturnData($cfg.url + $(this).text());
			return false;
		}).click(function() {
			$cfg.file = $(this).text();
			$('>div.l', statusDiv).html('<div class="cutName"><a href="' + $cfg.url + $cfg.file + '" target="_urlFile">' + $cfg.url + $cfg.file + '</a></div><div>'+$lang.fileSize+': '+$('.size', div).text()+'</div><div>'+$lang.fileDate+': '+$('.date', div).text()+'</div>');
