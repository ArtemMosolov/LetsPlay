...
<div class="container">
	<div class="row">
		<br>
		<c:choose>
			<c:when test="${not empty ArtistError}">
				<h1 align="center">
			...
			<c:otherwise>
				<div class="jumbotron">
					...
					<div class="row">
						<div class="col-md-4">
							<c:if test="${artist.artistDescription.image.fileName  ne null }">
								<img id="picture"
									src='<lp:fileTag fileName="${artist.artistDescription.image.fileName}"/>'
									alt="Empty picture!">
							</c:if>
							<c:if test="${artist.artistDescription.image.fileName eq null }">
								<img id="picture"
									src="<c:url value= "/resourse/image/800x500.gif" />">
							</c:if>
						</div>
						<div class="col-md-8">
							<h2>${artist.nameArtist}</h2>
							<p>${artist.artistDescription.artistDescription}</p>
						</div>
					</div>
				</div>
			</c:otherwise>
	...
	<div class="col-lg-12">
		<c:choose>
			<c:when test="${not empty SongsError}">
				<h1 align="center">
					<fmt:message key="NO_POPULAR_SONGS" bundle="${lang}" />
				</h1>
			</c:when>
			<c:otherwise>
				<h1 align="center">
					<fmt:message key="MOST_POPULAR_SONGS_FOR_CURRENT_ARTIST"
						bundle="${lang}" />
				</h1>
				<br>
				...
				<table class="table table-striped">
					...
					<c:forEach items="${popularSongsForCurrentArtist}"
						var="popularSong" varStatus="status">
						<tr align="center">
